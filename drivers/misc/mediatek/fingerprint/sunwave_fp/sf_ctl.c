...
#endif
    /* beanpod ISEE2.7 */
#if (SF_BEANPOD_COMPATIBLE_V2_7||((SF_PLATFORM_SEL == SF_TEE_BEANPOD)&&(SF_COMPATIBLE_SEL==SF_COMPATIBLE_BEANPOD_V2)))
    {
...
static int sf_read_sensor_id(void)
{
    ...
    do {
        ...
        xprintk(KERN_ERR, "readbuf[2]:%d,readbuf[3]:%d,readbuf[4]:%d,readbuf[5]:%d", readbuf[2], readbuf[3], \
                readbuf[4], readbuf[5]);
        ...
        xprintk(KERN_ERR, "readbuf[5]:%d", readbuf[5]);
        ...
        xprintk(KERN_ERR, "readbuf[4]:%d", readbuf[4]);
