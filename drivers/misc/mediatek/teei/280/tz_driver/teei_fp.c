...
int send_fp_command(void *buffer, unsigned long size)
{
	...
	if (buffer == NULL || size < 1)
		return -1;
	IMSG_ERROR("send_fp_command 111");
	if (context_initialized == 0) {
		memset(&context, 0, sizeof(context));
		IMSG_ERROR("send_fp_command 222");
		ret = ut_pf_gp_initialize_context(&context);
		IMSG_ERROR("send_fp_command 333");
		if (ret) {
	...
	IMSG_ERROR("send_fp_command 4444");
	ret = ut_pf_gp_transfer_data(&context, &uuid_fp, 1, buffer, size);
	IMSG_ERROR("send_fp_command 555");
	if (ret) {
