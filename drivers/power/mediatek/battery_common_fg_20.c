...
/* battery long charge protection */
#define CHARGING_LEVEL_Y 60
static u64 FULLY_CHARGED_X = 48*60*60*HZ; //48Hours
static bool g_charge_protect_trigger = false;
static u64 g_start_jiffies64 = 0;
				...
				enum power_supply_property psp, union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CHARGE_FULL:
		...
		break;
	case POWER_SUPPLY_PROP_CHARGE_PROTECT:
		val->intval = data->BAT_charge_protect; /* Long-charge protection */
		break;
	default:
		ret = -EINVAL;
		break;
...
/* CC: modify to have a full power supply status */
	...
	.BAT_batt_vol = 4200,
	.BAT_batt_temp = 22,
	.BAT_charge_protect = POWER_SUPPLY_CHARGE_PROTECT_LEAVE,
#else
...
ssize_t battery_GetBatteryChargecurrent(void)
{
	int temp = 0;
	signed int IBattery = battery_meter_get_battery_current();
	IBattery *= (battery_meter_get_battery_current_sign() ? 1 : (-1));
	temp = IBattery / 10; 
	if(temp < 0){
		temp = 0;
...
ssize_t battery_GetBatteryPercent(void)
{
	int bat_percent = -1;
	#ifdef CONFIG_WIND_ASUS_BATTERY_LIFE_SUPPORT
		if(BATLIFE_status.Normal2Batlife == 1 && BMT_status.UI_SOC2 >= 40){
			bat_percent = BMT_status.UI_SOC_BATLIFE;
		}
		else{
			bat_percent = BMT_status.UI_SOC2;	
		}
	#else
		bat_percent = BMT_status.UI_SOC2;
	#endif
...
{
	unsigned int charging_enable = true;
	battery_log(BAT_LOG_CRTI, "[battery_charge_enable] before cmd_discharging=%d\n", cmd_discharging);
	cmd_discharging = 0;
	charging_enable = true;
	adjust_power = -1;
	battery_charging_control(CHARGING_CMD_ENABLE, &charging_enable);
	battery_log(BAT_LOG_CRTI, "[battery_charge_enable] after cmd_discharging=%d\n", cmd_discharging);
}

void battery_charge_disable(void)
{
	unsigned int charging_enable = false;
	battery_log(BAT_LOG_CRTI, "[battery_charge_disable] before cmd_discharging=%d\n", cmd_discharging);
	cmd_discharging = 1;
	charging_enable = false;
	adjust_power = -1;
	battery_charging_control(CHARGING_CMD_ENABLE, &charging_enable);
	battery_log(BAT_LOG_CRTI, "[battery_charge_disable] after cmd_discharging=%d\n", cmd_discharging);
}

static void battery_charge_protect_leave(void)
{
	battery_log(BAT_LOG_CRTI, "before trigger=%d\n", g_charge_protect_trigger);
	g_charge_protect_trigger = false;
	g_start_jiffies64 = 0;
	battery_charge_enable();
	battery_main.BAT_charge_protect = POWER_SUPPLY_CHARGE_PROTECT_LEAVE;
	battery_log(BAT_LOG_CRTI, "======battery_charge_protect_leave\n");
}

static void battery_charge_protect(void)
{
	battery_log(BAT_LOG_CRTI, "======battery_charge_protect\n");
	battery_log(BAT_LOG_CRTI, "BAT_CAPACITY=%d\n", battery_main.BAT_CAPACITY);
	battery_log(BAT_LOG_CRTI, "charger_exist=%d\n", BMT_status.charger_exist);
	battery_log(BAT_LOG_CRTI, "charging_full=%d\n", bat_is_charging_full());
	battery_log(BAT_LOG_CRTI, "trigger=%d\n", g_charge_protect_trigger);
	battery_log(BAT_LOG_CRTI, "cmd_discharging=%d\n", cmd_discharging);
	battery_log(BAT_LOG_CRTI, "g_start_jiffies64=%llu, get_jiffies_64=%llu\n", g_start_jiffies64, get_jiffies_64());
	
	if (g_charge_protect_trigger == true) { //auto disable: unplug charger OR current battery level < [y]%
		if (KAL_FALSE == BMT_status.charger_exist  || CHARGING_LEVEL_Y > battery_main.BAT_CAPACITY) {
			battery_charge_protect_leave();
			power_supply_changed(&battery_main.psy);
			battery_log(BAT_LOG_CRTI, "======leave long charge protection\n");
		}
	} else { //auto  enable: if accumulated time for battery staying in fully charged but unused for [x] hours continuously
		if (KAL_TRUE == BMT_status.charger_exist && battery_main.BAT_CAPACITY >= 100) {
			if (0 == g_start_jiffies64) {
				 g_start_jiffies64 = get_jiffies_64();
				 battery_log(BAT_LOG_CRTI, "g_start_jiffies64 = %llu\n", g_start_jiffies64);
			 } else {
			 	if (time_after64(get_jiffies_64(), g_start_jiffies64 + FULLY_CHARGED_X)) {
					g_start_jiffies64 = 0;
					g_charge_protect_trigger = true;
					battery_charge_disable();
					battery_main.BAT_charge_protect = POWER_SUPPLY_CHARGE_PROTECT_ENTER;
					power_supply_changed(&battery_main.psy);
					battery_log(BAT_LOG_CRTI, "======enter long charge protection\n");
			 	}
			 }
		} else
			g_start_jiffies64 = 0;
...
static int charge_protect_read(struct seq_file *buf, void *data)
{
	char *charge_protect_text[] = { "Unknown", "Enable", "Disable" };
	seq_printf(buf, "%s\n", charge_protect_text[battery_main.BAT_charge_protect]);
	return 0;
}

static ssize_t charge_protect_write(struct file *file,const char __user *buffer,size_t count,loff_t *pos)
{
	battery_charge_protect_leave();
	return count;
}

static int charge_protect_proc_open(struct inode *inode, struct file *file)
{
	return single_open(file, charge_protect_read, NULL);
}

static const struct file_operations charge_protect_fops = {
	.open = charge_protect_proc_open,
	.read = seq_read,
	.write = charge_protect_write,
};
...
static int mt_batteryNotify_probe(struct platform_device *dev)
{
	...
	proc_create("charge_protect", S_IRUGO | S_IWUGO, battery_dir, &charge_protect_fops);
	battery_log(BAT_LOG_CRTI, "proc_create charge_protect_fops\n");
